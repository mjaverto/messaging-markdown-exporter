...
function checkSource(source: string, input: DoctorInput, warnings: string[]): void {
  if (source === "imessage") {
    ...
    if (!fs.existsSync(input.dbPath)) warnings.push(`Messages database not found at ${input.dbPath}. Full Disk Access or path may be wrong.`);
    warnings.push("Reminder: the terminal or app running this tool needs Full Disk Access to read ~/Library/Messages/chat.db.");
    return;
  }
  if (source === "whatsapp") {
    if (!hasCommand("sqlite3")) warnings.push("sqlite3 is not on PATH. The WhatsApp adapter shells out to sqlite3 to read ChatStorage.sqlite.");
    if (input.whatsappDbPath && !fs.existsSync(input.whatsappDbPath)) {
      warnings.push(`WhatsApp database not found at ${input.whatsappDbPath}. WhatsApp Desktop must be installed and Full Disk Access granted.`);
    }
    return;
  }
  if (source === "signal") {
    if (input.signalDbPath && !fs.existsSync(input.signalDbPath)) {
      warnings.push(`Signal database not found at ${input.signalDbPath}. Signal Desktop must be installed and Full Disk Access granted.`);
    }
    if (input.signalConfigPath && !fs.existsSync(input.signalConfigPath)) {
      warnings.push(`Signal config not found at ${input.signalConfigPath}; required to derive the SQLCipher key.`);
    }
    return;
  }
  if (source === "telegram") {
    if (input.telegramConfigDir && !fs.existsSync(input.telegramConfigDir)) {
      warnings.push(`Telegram config dir not found at ${input.telegramConfigDir}. Run 'imessage-to-markdown telegram-login' first.`);
    }
    return;
  }
