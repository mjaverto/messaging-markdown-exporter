...
const SYSTEM_CHAT_RE = /(verification code|otp|2fa|do not reply|no-reply|automated|alert|notification)/i;
...
export function looksLikeSystemChat(name: string | null | undefined, participants: string[]): boolean {
  return SYSTEM_CHAT_RE.test([name, ...participants].filter(Boolean).join(" "));
}

export function slugForChat(name: string | null | undefined, participants: string[], fallback: string): string {
